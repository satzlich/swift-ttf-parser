...
struct DeviceTable: SafeDecodable {
    ...
    init?(_ bytes: UnsafeBufferPointer<UInt8>) {
        ...
        self.startSize = UInt16.decode(bytes.baseAddress! + _Offsets.startSize)
        self.endSize = UInt16.decode(bytes.baseAddress! + _Offsets.endSize)
        self.deltaFormat = UInt16.decode(bytes.baseAddress! + _Offsets.deltaFormat)

        // detlaValue
        do {
            let bytes = bytes.rebase(_Offsets.deltaValue)
    ...
    public static let minWidth: Int = _Offsets.deltaValue
